...
const getGoogleSDK = () =>
  new Promise((resolve, reject) => {
    https.get('https://dl.google.com/dl/cloudsdk/channels/rapid/downloads/google-cloud-sdk-155.0.0-darwin-x86_64.tar.gz', resp => {
...
const check = () => {
  if (!shell.which('gcloud')) {
    ...
    return getGoogleSDK()
    .then(fp => {
      shell.exec(`${path.join(fp, 'install.sh')} -q --additional-components kubectl`, { silent: true });
      shell.rm('-rf', fp);
  ...
  if (!/kubectl/.test(versionInfo)) {
    ...
    shell.exec('echo "y" | gcloud components install kubectl', { silent: true });
  }
