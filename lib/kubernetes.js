...
const kubernetes = {
  ...
  pods(appName) {
    return JSON.parse(execSync(`kubectl get pods -o json --selector=app=${appName}`).toString());
  },

  exec(podName, containerName, args) {
    execSync(`kubectl exec ${podName} -it ${containerName ? `-c ${containerName}` : ''} ${args.join(' ')}`, {
      stdio: 'inherit',
    });
  },
};
